...
import MealPlanEditModal from "../Modals/MealPLan/MealPlanEditModal";
import MealPlanViewModal from "../Modals/MealPLan/MealPlanViewModal";
import MealPlanDeleteModal from "../Modals/MealPLan/MealPlanDeleteModal";
...
return(
  <div className="mb-4 h-full w-full px-1 md:px-4 md:h-1/3 md:w-1/3 xl:h-1/4 xl:w-1/4 py-4">
   
      <div className="border-gray-100 relative cursor-pointer overflow-hidden rounded-xl  dark:border dark:border-slate-500 bg-white dark:bg-inherit shadow-lg ">
        <div className="relative h-70 overflow-hidden md:h-60 lg:h-67">
          <img
            className="h-full w-full object-cover"
            src={mealPlan.image}
            alt={mealPlan.name}
          />
          <div className="absolute left-0 top-6 rounded-r-xl bg-orange-500 bg-opacity-75 px-4  py-2 text-white">
            <span className="block text-xs font-medium uppercase">
              {mealPlan.name}
            </span>
            <p className="block text-xs">
              <span className="font-mediumuppercase text-xs">
                {mealPlan.calories}
              </span>{" "}
              calories
            </p>
        ...
        <div className="h-auto p-4 md:h-1/5 lg:h-14">
          <h2 className="mb-2 text-center text-sm sm:text-lg   font-bold text-green-600">
            {mealPlan.name}
          </h2>
        </div>
        <div className="flex justify-center  border-t p-4">
          <div className="flex items-center space-x-3.5">
            <button id="viewMealPLan" className="hover:text-primary" onClick={() => setViewModalOpen(true)}>
              <svg
            ...
            <button id="editMealPlan" className="hover:text-primary" onClick={() => setEditModalOpen(true)}>
              <svg
              className="fill-current"
                fill="none"
                height="20"
            ...
            <button id="deleteMealPlan" className="hover:text-primary" onClick={() => setDeleteModalOpen(true)}>
              <svg
                className="fill-current"
                width="20"
                height="20"
