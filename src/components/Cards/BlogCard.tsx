...
const BlogCard: React.FC<BlogCardProps> = ({ Blog }) => {
  ...
  const handleEdit = (updatedData: typeof Blog) => {
  ...
  return (
    <div>
     <li key={Blog.id} className="relative flex flex-col items-start sm:flex-row xl:flex-col">
            <div className="order-1 sm:ml-6 xl:ml-0">
              <h3 className="mb-3 border-b pb-2">
                <span className="mb-1 block text-xl font-bold leading-6 text-green-700">
                  {Blog.title}
                </span>
                {Blog.subtitle}
              </h3>
              <div className="prose prose-slate prose-sm overflow-hidden max-h-20">
                <p>
                {Blog.content}
                </p>
              </div>
              <div className="border-b-1 group mt-6 inline-flex h-9 items-center whitespace-nowrap rounded-full px-3 text-sm font-semibold focus:outline-none focus:ring-2 ">
                <div className="flex justify-center p-4">
                  <div id="view_blog" className="flex   space-x-8 ">
                    <button
            ...
            <img
              src={Blog.image}
              alt=""
              className="mb-6 w-full rounded-lg bg-slate-50 shadow-md sm:mb-0 sm:w-[17rem] xl:mb-6 xl:w-full"
              width="1216"
              height="640"
            />
          </li>
      ...
      {/* Modals */}
      {viewModalOpen && (
        <ViewModal isOpen={viewModalOpen} onClose={() => setViewModalOpen(false)}  Blog={Blog} />
      )}
      {editModalOpen && (
        <EditModal isOpen={editModalOpen} onClose={() => setEditModalOpen(false)} onEdit={handleEdit} Blog={Blog} />
      )}
      {deleteModalOpen && (
        <DeleteModal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)} onDelete={handleDelete}  Meal={Meal} />
      )}
    </div>
  );
};
